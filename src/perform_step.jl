...
function OrdinaryDiffEqCore.perform_step!(integ, cache::EKCache, repeat_step=false)
    ...
    # Backward kernels are needed by the RTS smoother (`smoother=:rts`, only when
    # `smooth=true`) and by Fenrir's own backward pass (fit_pnsolution_to_data!, opt-in via
    # `save_backward_kernels=true`). The MBF smoother (smoother=:mbf) does not need them.
    if (integ.alg.smooth && integ.alg.smoother == :rts) || integ.alg.save_backward_kernels
