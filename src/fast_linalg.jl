...
Returns an `UpperTriangular` wrapping a view of the first `min(m, n)` rows of `A`, after
zeroing the entries below the diagonal of that view in-place. The result therefore aliases
`A`, and `A` is modified.
...
function getupperright!(A)
    m, n = size(A)
    return UpperTriangular(triu!(view(A, 1:min(m, n), 1:n)))
end
