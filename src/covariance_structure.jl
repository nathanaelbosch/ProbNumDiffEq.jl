...
factorized_zeros(C::BlockDiagonalCovariance{T}, sizes...) where {T} = begin
    ...
    return BlocksOfDiagonals([Array{T}(calloc, (s ÷ C.d for s in sizes)...) for _ in 1:C.d])
end
factorized_similar(C::BlockDiagonalCovariance{T}, size1, size2) where {T} = begin
    for s in (size1, size2)
        @assert s % C.d == 0
    end
    return BlocksOfDiagonals([similar(Matrix{T}, size1 ÷ C.d, size2 ÷ C.d) for _ in 1:C.d])
end
