...
abstract type AbstractSemiLinearModel{IIP} end
struct SemiLinearModel{T1,T2,T3,IIP} <: AbstractSemiLinearModel{IIP}
    A::T1
    b::T2
    C::T3
end
SemiLinearModel(A, b, C) = SemiLinearModel{typeof(A),typeof(b),typeof(C),true}(A, b, C)
(z::SemiLinearModel)(out, Y, p, t) = begin
    z.b(out, z.C * Y, p, t)
    out .= z.A * Y - out
end
...
function DiffEqBase.init(
    ...
    gamma=OrdinaryDiffEq.gamma_default(alg),
    abstol=nothing,
    reltol=nothing,
    qmin=OrdinaryDiffEq.qmin_default(alg),
    qmax=OrdinaryDiffEq.qmax_default(alg),
    qsteady_min=OrdinaryDiffEq.qsteady_min_default(alg),
    qsteady_max=OrdinaryDiffEq.qsteady_max_default(alg),
    beta1=nothing,
    beta2=nothing,
    qoldinit=OrdinaryDiffEq.isadaptive(alg) ? 1 // 10^4 : 0,
    controller=nothing,
    fullnormalize=true,
    failfactor=2,
    maxiters=adaptive ? 1000000 : typemax(Int),
    internalnorm=OrdinaryDiffEq.ODE_DEFAULT_NORM,
    internalopnorm=LinearAlgebra.opnorm,
    isoutofdomain=OrdinaryDiffEq.ODE_DEFAULT_ISOUTOFDOMAIN,
    unstable_check=OrdinaryDiffEq.ODE_DEFAULT_UNSTABLE_CHECK,
    verbose=true,
    ...
    progress_steps=1000,
    progress_name="ODE",
    progress_message=OrdinaryDiffEq.ODE_DEFAULT_PROG_MESSAGE,
    userdata=nothing,
    ...
    d_discontinuities_internal =
        OrdinaryDiffEq.initialize_d_discontinuities(tType, d_discontinuities, tspan)
    ...
    if OrdinaryDiffEq.uses_uprev(_alg, adaptive) || calck
    ...
    if controller === nothing
        controller = OrdinaryDiffEq.default_controller(
            _alg,
            cache,
            qoldinit,
    ...
    id = OrdinaryDiffEq.InterpolationData(f, timeseries, ts, ks, dense, cache)
    ...
    dtchangeable = OrdinaryDiffEq.isdtchangeable(_alg)
    q11 = QT(1)
    success_iter = 0
    erracc = QT(1)
    dtacc = tType(1)
    ...
    integrator = OrdinaryDiffEq.ODEIntegrator{
        ...
        cacheType,
        typeof(opts),
        OrdinaryDiffEq.fsal_typeof(_alg, rate_prototype),
