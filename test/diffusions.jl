...
@testset "Test the different diffusion models" begin
    ...
    @testset "Time-Varying Diffusion" begin
        sol = solve(prob, EKF0(diffusionmodel=:dynamic), adaptive=false, dt=1e-4)
        @test sol.u ≈ true_sol.(sol.t)
    end

    @testset "Time-Fixed Diffusion" begin
        sol = solve(prob, EKF0(diffusionmodel=:fixed), adaptive=false, dt=1e-4)
        @test sol.u ≈ true_sol.(sol.t)
    end

    @testset "Time-Varying Diagonal Diffusion" begin
        sol = solve(prob, EKF0(diffusionmodel=:dynamicMV), adaptive=false, dt=1e-4)
        @test sol.u ≈ true_sol.(sol.t)
    end

    @testset "Time-Fixed Diagonal Diffusion" begin
        sol = solve(prob, EKF0(diffusionmodel=:fixedMV), adaptive=false, dt=1e-4)
        @test sol.u ≈ true_sol.(sol.t)
    end

    @testset "Time-Fixed Diffusion MAP" begin
        sol = solve(prob, EKF0(diffusionmodel=:fixedMAP), adaptive=false, dt=1e-4)
        @test sol.u ≈ true_sol.(sol.t)
    end
